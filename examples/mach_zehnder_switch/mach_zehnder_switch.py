...
for power_sweep in power_values:
    ...
    plt.plot(circuit.sim_result.c/circuit.sim_result.f_*1e6, 20*np.log10(circuit.sim_result.s_[:,1,0]))

plt.title("MZI power sweep w/ tunable WG component")
plt.legend(["%s W"%(each) for each in [0, 22e-3, 24e-3, 26e-3]])
plt.ylabel("dB")
plt.xlabel("um")
plt.xlim(np.min(circuit.sim_result.c/circuit.sim_result.f_*1e6), np.max(circuit.sim_result.c/circuit.sim_result.f_*1e6))
plt.show()
